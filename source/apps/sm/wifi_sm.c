...
int neighbor_response(wifi_provider_response_t *provider_response)
{
    ...
    if(survey_type_to_str(survey_type) == NULL) {
        wifi_util_error_print(WIFI_SM, "SJY %s:%d: survey_type_str is NULL\r\n", __func__, __LINE__);
        return RETURN_ERR;
    }
    wifi_util_error_print(WIFI_SM, "SJY %s:%d: survey_type_str is %s\r\n", __func__, __LINE__, survey_type_str);

    wifi_util_error_print(WIFI_SM, "SJY %s:%d: radio_index : %d\r\n", __func__, __LINE__, radio_index);

    if(radio_index_to_radio_type_str(radio_index) == NULL) {
        wifi_util_error_print(WIFI_SM, "SJY %s:%d: radio_type_str is NULL\r\n", __func__, __LINE__);
        return RETURN_ERR;
    }
    wifi_util_error_print(WIFI_SM, "SJY %s:%d: radio_type_str is %s\r\n", __func__, __LINE__, radio_type_str);
